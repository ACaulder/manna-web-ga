...
<div class="grid grid-cols-12 gap-6 p-8 min-h-screen">
  <aside class="col-span-4 space-y-4 sticky top-0 self-start">
    <div class="flex items-center justify-between">
      ...
      <button class="px-3 py-1 rounded bg-zinc-800 hover:bg-zinc-700" on:click={loadTopics}>Reload</button>
    </div>
    ...
    {#each clusters as c (c.id)}
      <button
        class="w-full text-left rounded border border-zinc-700/60 p-3 hover:bg-zinc-900/40 {selectedCluster?.id === c.id ? 'ring-2 ring-emerald-500/40 bg-zinc-900/60' : ''}"
        on:click={() => { selectedCluster = c; selectedDoc = null; summary = ''; filter=''; }}
      >
  ...
  <main class="col-span-8 space-y-4">
    ...
    {#if selectedDoc}
      ...
      {:else}
        <pre class="whitespace-pre-wrap text-sm border border-zinc-700/60 rounded p-3">{summary}</pre>
      {/if}
    {/if}
  </main>
</div>
...
<style>
  .grid { font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, sans-serif; }
</style>