    ...
    <div class="rounded-2xl border border-neutral-800 bg-neutral-950/60 p-4 space-y-4">
      <div class="flex items-center justify-between">
        ...
        <span class="text-xs text-neutral-400">
          Sorted by importance & appearances
        </span>
      </div>
      ...
      {:else}
        <div class="space-y-2 max-h-[28rem] overflow-auto pr-1">
          {#each people as person}
            <div class="flex items-start justify-between rounded-xl border border-neutral-800/80 bg-neutral-900/60 px-3 py-2">
              <div class="space-y-1">
                ...
                {#if person.commitments.length > 0}
                  <div class="text-xs text-amber-300">
                    {person.commitments.length} open commitment{person.commitments.length === 1 ? '' : 's'}
                  </div>
                {/if}
              </div>
              ...
              <div class="flex flex-col items-end gap-1">
                {#if person.commitments.length > 0}
                  <span class="rounded-full bg-amber-500/10 px-2 py-0.5 text-[0.7rem] text-amber-300">
                    Needs attention
                  </span>
                {/if}
      ...
      {:else}
        <div class="space-y-2 max-h-[28rem] overflow-auto pr-1">
          {#each openCommitments as c}
            <article class="rounded-xl border border-neutral-800/80 bg-neutral-900/60 px-3 py-2 space-y-1">
              <div class="flex items-center justify-between gap-2">
                <div class="text-sm font-medium">
                  {c.action}
                </div>
                <span class="text-[0.7rem] rounded-full bg-neutral-800 px-2 py-0.5 text-neutral-300">
                  {c.owner}
                </span>
              </div>
              {#if c.context}
